...
case class Cancel(timestamp: Long, order: Order) extends OrderBookRequest

case class Amend(timestamp: Long, order: Order, newPrice: Option[Double], newQty: Option[Long]) extends OrderBookRequest

abstract class OrderBookResponse

case class Filled(timestamp: Long, price: Double, qty: Long, order: Array[Order]) extends OrderBookResponse
...
case class Canceled(timestamp: Long, reason: String, order: Order) extends OrderBookResponse
...
case class Order(timestamp: Long, tradeID: String, symbol: String, var qty: Long, isBuy: Boolean, var price: Option[Double]) extends OrderBookRequest
...
class OrderBook(symbol: String) {
  ...
  def processOrderBookRequest(request: OrderBookRequest): Unit = request match {
    case order: Order => {
      ...
      if (!isOK) this.transactionObserver(Rejected(currentTime, message.getOrElse("N/A"), order))
        ...
        val orderBookOrder = Order(order.timestamp, order.tradeID, order.symbol, order.qty, order.isBuy, order.price)
        processOrder(orderBookOrder)
      }
    }
    case cancel: Cancel => {
  ...
  def processOrder(orderBookOrder: Order) {
    val currentTime = System.currentTimeMillis
    ...
    if (orderBookOrder.price.isDefined) {
      ...
      if (oppositeQ.isEmpty) this.transactionObserver(Rejected(currentTime, "No opposing orders in queue", orderBookOrder))
      else matchOrder(orderBookOrder, oppositeQ)
    }
  }

  private def validateOrder(order: Order): (Boolean, Option[String]) = (true, None)
  ...
  private def matchOrder(order: Order, oppositeQ: PriorityQueue[Order]): Unit = {
    ...
    if (order.qty < oppositeOrder.qty) {
      ...
      this.transactionObserver(Filled(currentTime, order.price.get, order.qty, Array(order, oppositeOrder)))
